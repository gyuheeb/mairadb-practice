...
-- 프로젝션(projection)
-- 예제 2:  emplyees 테이블에서 직원의 이름, 성별,입사일을 출력
select dept_no,dept_name from departments;

-- as (alias, 생략가능)
-- 예제 3: emplyees 테이블에서 직원의 이름,성별, 입사일을 출력
...
select title from titles;
 -- 예제 2: titles 테이블에서 직급은 어떤 것들이 있는 지 지급이름 검색
select distinct (title) from titles;
 
 -- where절(row를 조건에 맞게 선택한다)
...
-- 에제 2: 논리연산자 :employees 테이블에서 1989년 이전에 입사한 여직원의 이름, 입사일을 출력
...
select first_name, hire_date from employees where hire_date between '1989-01-01'and '1989-12-31';
 ...
 -- Order by 
 -- 예제 1: employees 테이블에서 직원의 전체 이름, 성별, 입사일 순으로 출력
 select concat(first_name, '' ,last_name), gender, hire_date from employees order by hire_date asc; 
 -- 에제 2: salaries 테이블에서 2001년 월급을 가장 높은순으로 사번, 월급순으로 출력
